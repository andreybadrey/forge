...
public class GuiAlchemistMenu  extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
    ...
    public GuiAlchemistMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        super(Menus.ALCHEMIST.get(), id);
        ...
        for (int si = 0; si < 3; ++si)
            for (int sj = 0; sj < 9; ++sj)
                this.addSlot(new Slot(inv, sj + (si + 1) * 9, 112 + sj * 20,  324 + si * 20));
        for (int si = 0; si < 9; ++si)
            this.addSlot(new Slot(inv, si, 112 + si * 20, 386));
    }
